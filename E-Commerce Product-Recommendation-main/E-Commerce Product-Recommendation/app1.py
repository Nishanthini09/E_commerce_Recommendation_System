...
PAGE_ICON =  "C:\\Users\\HAI\\Downloads\\E-Commerce Product-Recommendation-main\\E-Commerce Product-Recommendation-main\\amazon-512.webp"        #"🛒"  # Using a shopping cart emoji
st.set_page_config(page_title=PAGE_TITLE, page_icon=PAGE_ICON, layout="wide")

# Add Amazon image at the top
st.markdown(
    """
    <style>
    .reportview-container {
        background: url("https://upload.wikimedia.org/wikipedia/commons/a/a9/Amazon_logo.svg");
        background-size: 50%;
...
def display_selected_product_details(selected_product_details):
    ...
    st.write(f"**Rating:** {selected_product_details['rating']} ({selected_product_details['rating_count']} reviews)")
    ...
    for row in range(num_rows):
        for col in range(num_columns):
            ...
            with columns[col]:
                ...
                st.write(f"**Rating:** {recommendation['rating']} ({recommendation['rating_count']} reviews)")
...
def main():
    ...
    logo_image = 'C:\\Users\\HAI\\Downloads\\E-Commerce Product-Recommendation-main\\E-Commerce Product-Recommendation-main\\Amazon-app-Logo.png'
    ...
    selected_category = st.sidebar.selectbox('Select a category:', sorted(data['category'].unique()))
    filtered_data = data[data['category'] == selected_category]

    st.sidebar.title("Product Recommendations")

    # Create an input field for the user to enter the product ID    
    product_id_encoder = LabelEncoder()
    selected_product_name = st.sidebar.selectbox('Select a product:', sorted(filtered_data['product_name'].values))
    ...
    if st.button("Get Your Recommendations"):
